...
func getLatestReleaseName(url string) (string, error) {
	...
	for _, rel := range t {
		...
		if ok, _ := regexp.MatchString(`^v?1[.]`, rel.TagName); ok {
			// only allow "1.xxx" or "v1.xxx" to be pulled by this tool
			return rel.TagName, nil
		}
	}

	return "", fmt.Errorf("no 1.x, non-prerelease releases found at %q", url)
}
